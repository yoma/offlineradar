/**
 * Fase 6: non-speeddate expansion + Source Map updates + publish approved.
 * Usage: npm run import:phase6-expansion
 */
import {
...
import {
  buildPhase6ExpansionBatch,
  eligibilityJsonFor,
  PHASE6_SOURCE_MAP_UPSERTS,
  type Phase6Item,
} from "../data/pilot/phase6-expansion-batch";
...
async function upsertBatchItem(item: Phase6Item) {
...
async function main() {
  ...
  const batch = buildPhase6ExpansionBatch();
  ...
  for (const seed of PHASE6_SOURCE_MAP_UPSERTS) {
    const organizerSlugGuess = seed.name.toLowerCase().includes("villa")
      ? "villavibes"
      : seed.name.toLowerCase().includes("ontvlam")
        ? "ontvlam"
        : seed.name.toLowerCase().includes("will you")
          ...
          : seed.name.toLowerCase().includes("lucky")
            ...
            : seed.name.toLowerCase().includes("sircle")
              ? "the-sircle"
              : null;
    let resolvedOrgId = organizerSlugGuess
  ...
  console.log("\nOK: phase6 non-speeddate expansion.");
}
