/**
 * Fase 5: import expansion + seed Source Map + publish approved.
 * Usage: npm run import:phase5-expansion
 */
import {
...
import {
  buildPhase5ExpansionBatch,
  eligibilityJsonFor,
  PHASE5_SOURCE_MAP_SEED,
  type Phase5Item,
} from "../data/pilot/phase5-expansion-batch";
...
async function upsertBatchItem(item: Phase5Item) {
...
async function main() {
  ...
  const batch = buildPhase5ExpansionBatch();
  ...
  // Keep Padeldate under_review (conflict unresolved)
  ...
  for (const seed of PHASE5_SOURCE_MAP_SEED) {
    const organizerSlugGuess = seed.name
      .toLowerCase()
      .includes("smartvibes")
      ? "smartvibes"
      : seed.name.toLowerCase().includes("lespeeddating")
        ? "lespeeddating"
        : seed.name.toLowerCase().includes("will you")
          ? "will-you-date-me"
          : seed.name.toLowerCase().includes("love doctor")
            ? "the-love-doctor"
            : seed.name.toLowerCase().includes("embodied")
              ? "embodied-dating-club"
              : seed.name.toLowerCase().includes("conscious")
                ? "wim-vandenbergh-conscious-dating"
                : seed.name.toLowerCase().includes("sircle")
                  ? "the-sircle"
                  : seed.name.toLowerCase().includes("s-plus")
                    ? "s-plus-vzw"
                    : seed.name.toLowerCase().includes("house of") ||
                        seed.name.toLowerCase().includes("how to be")
                      ? "how-to-be-single"
                      : seed.name.toLowerCase().includes("lucky")
                        ? "lucky-lemon"
                        : null;
    const organizerId = organizerSlugGuess
      ? orgIds.get(organizerSlugGuess) ?? null
      : null;
    // Resolve known organizers from DB if missing in this batch
    let resolvedOrgId = organizerId;
    if (!resolvedOrgId && organizerSlugGuess) {
      const rows = await sql`
        SELECT id FROM organizers WHERE slug = ${organizerSlugGuess} LIMIT 1
      `;
      resolvedOrgId = (rows[0] as { id: string } | undefined)?.id ?? null;
    }
    const result = await upsertCatalogSourceByUrl({
      ...seed,
      organizerId: resolvedOrgId,
    });
  ...
  `;
  const sourceCount = await sql`SELECT count(*)::int AS n FROM catalog_sources`;
    ...
    JSON.stringify(
      {
        ...
        catalogSources: (sourceCount[0] as { n: number }).n,
        sourcesCreated,
      },
      null,
      2,
  ...
  console.log("\nOK: phase5 expansion + source map.");
}
